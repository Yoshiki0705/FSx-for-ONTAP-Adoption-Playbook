...
| 8 | Whether request count, HTTP error rate, or request latency is available per S3 Access Point | **The reviewed CloudWatch pages do not list Access Point-level series.** Keep these separate from aggregate storage telemetry. [CloudTrail can record requests through Access Points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-points-monitoring-logging.html) when S3 data-event logging is configured, but it does not provide the same metric series. |
