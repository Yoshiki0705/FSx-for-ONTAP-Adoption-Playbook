...
| **Serving a volume already in use over SMB to NFS clients as well** | [Adding NFS to a volume already serving SMB needs no clone](domains/multiprotocol-identity/notes/adding-a-protocol-does-not-need-a-clone.md) | [Security style determines the permission model](domains/multiprotocol-identity/notes/security-style-and-permission-evaluation.md) |
...
| **Unsure whether block is needed or Amazon EBS is enough** | [Block storage options comparison](../ja/reference/comparison/block-storage-options.md) (日本語) | [When shared block changes the design](domains/block-storage/notes/when-shared-block-changes-the-design.md) |
| **Comparing cost against an existing Amazon EBS estate** | [What stops Amazon EBS being the cheaper answer](domains/block-storage/notes/when-ebs-stops-being-the-cheaper-answer.md) | [Block storage quickstart](domains/block-storage/quickstart.md) |
| **Analysing data on a LUN over files or the S3 API without touching production** | [LUN contents do not reach the file protocols](domains/block-storage/notes/lun-contents-do-not-reach-file-protocols.md) | [Routes for moving block data to files](../ja/reference/comparison/block-to-file-routes.md) (日本語) |
| **Moving a volume to another SVM, or checking whether a FlexClone can be moved** | [`volume rehost` changes the owning SVM, not the contents](domains/block-storage/notes/volume-rehost-changes-ownership-not-contents.md) | [Routes for moving block data to files](../ja/reference/comparison/block-to-file-routes.md) (日本語) |
