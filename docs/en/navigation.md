...
| **Serving a volume already in use over SMB to NFS clients as well** | [Adding NFS to a volume already serving SMB needs no clone](../ja/domains/multiprotocol-identity/notes/adding-a-protocol-does-not-need-a-clone.md) (日本語) | [Security style determines the permission model](domains/multiprotocol-identity/notes/security-style-and-permission-evaluation.md) |
| **Checking permissions from the NFS side, or explaining a denial** | [The NFS-side permission view does not match the enforced outcome](domains/multiprotocol-identity/notes/nfs-side-view-does-not-explain-ntfs-denials.md) | [Security style determines the permission model](domains/multiprotocol-identity/notes/security-style-and-permission-evaluation.md) |
...
| **Unsure whether Amazon EFS is enough or FSx for ONTAP is needed** | [Which AWS file storage decision tree](../ja/reference/decision-trees/file-storage-selection.md) (日本語) | [File storage options comparison](../ja/reference/comparison/file-storage-options.md) (日本語) |
...
| **Comparing cost against an existing Amazon EBS estate** | [What stops Amazon EBS being the cheaper answer](../ja/domains/block-storage/notes/when-ebs-stops-being-the-cheaper-answer.md) (日本語) | [Block storage quickstart](domains/block-storage/quickstart.md) |
| **Analysing data on a LUN over files or the S3 API without touching production** | [LUN contents do not reach the file protocols](../ja/domains/block-storage/notes/lun-contents-do-not-reach-file-protocols.md) (日本語) | [Routes for moving block data to files](../ja/reference/comparison/block-to-file-routes.md) (日本語) |
| **Moving a volume to another SVM, or checking whether a FlexClone can be moved** | [`volume rehost` changes the owning SVM, not the contents](../ja/domains/block-storage/notes/volume-rehost-changes-ownership-not-contents.md) (日本語) | [Routes for moving block data to files](../ja/reference/comparison/block-to-file-routes.md) (日本語) |
...
| **Combining a product you already run with FSx for ONTAP** | [ISV and SaaS solution map by problem](../ja/reference/isv-solution-map.md) (日本語) | [Candidates that did not meet the bar](../ja/reference/isv-solution-map.md#掲載基準を満たさないもの) (日本語) |
...
| [ISV and SaaS solution map by problem](../ja/reference/isv-solution-map.md) (日本語) | You want to enter from a problem and see which products have a published combination, and which of them this repository has a design note for |
