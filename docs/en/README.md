...
| [LUN layout decides recovery granularity](domains/block-storage/notes/lun-layout-decides-recovery-granularity.md) | One LUN per volume is not a general recommendation. `lun move` was non-disruptive and kept the WWID |
...
| [Paths are the failover mechanism](domains/block-storage/notes/paths-are-the-failover-mechanism.md) | Following the procedure produced 16 paths per LUN, and the connection script is not idempotent |
...
| [When shared block changes the design](domains/block-storage/notes/when-shared-block-changes-the-design.md) | Amazon EBS is the straightforward choice for a single attachment. The million-IOPS figure is ten file systems combined |
| [Kubernetes block volumes meet the volume limit](domains/block-storage/notes/kubernetes-block-volumes-and-the-volume-limit.md) | What runs out is volumes, not capacity, and the driver choice sets that ceiling |
