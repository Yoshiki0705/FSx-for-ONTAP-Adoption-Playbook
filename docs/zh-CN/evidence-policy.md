...
| `documented` | 厂商或 AWS 官方文档中有记载。**或者，拥有测量环境的其他项目的记录中有记载**（见下文） | `source`（URL 或文档名） | 可作为一次信息对待。但需注意版本与区域差异 |
...
**转载其他项目测量的数值同样是 `documented`。** `verified` 主张的是「本仓库的作者在该环境中复现过」，引用并不满足这一点。**但引用附带 3 个特有条件。**

| 条件 | 理由 |
|---|---|
| **完整转载全部测量条件** | 缺少条件的数值无法用于设计。世代、容量、IOPS、缓存状态、客户端实例类型、并发度都要写 |
| **连引用来源自称未测量的范围一并转载** | **引用可以只取方便的部分。** 丢掉来源列出的空白，就等于伪装确定性 |
| **登记到[跨项目引用索引](../ja/reference/cross-repo-index.md)（日本語）** | **引用会静默腐烂。** 来源撤回主张时，这边的表述不会改变。登记后 `make cross-repo-external` 会检出撤回 |
