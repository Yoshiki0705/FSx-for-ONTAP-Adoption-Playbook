...
| S3 Access Points network, troubleshooting and references | [Configuring network access](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/configuring-network-access-for-s3-access-points.html); [Troubleshooting S3 access point issues](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/troubleshooting-access-points-for-fsxn.html); [Referencing access points](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/referencing-access-points-for-fsxn.html) | The same reader surfaces plus `examples/client-access/` | Endpoint requirements are now scoped by caller location. The controlled 2026-08-17/18 `ap-northeast-1` in-VPC measurement remains evidence that an Internet-origin access point can use an S3 gateway endpoint when the subnet route table selects it. The example was already scoped to VPN ingress and remains unchanged. |
