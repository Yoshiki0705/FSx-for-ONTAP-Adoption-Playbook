...
| Amazon CloudWatch percentile availability | AUDITED WITH OPEN ACCESS POINT METRICS | The p99 limitation is scoped to volume read/write/metadata operation-time/count pairs whose valid statistic is `Sum`. Applicability of Amazon S3 Access Point-filtered request metrics to FSx for ONTAP-backed Access Points remains open. |
...
| Volume operation latency and other CloudWatch metric families | [Volume metrics](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/volume-metrics.html); [File system metrics](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/file-system-metrics.html); [Second-generation file system metrics](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/so-file-system-metrics.html); [Monitoring and logging Access Points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-points-monitoring-logging.html) | Root and English hubs; JA/EN performance notes and module hubs; JA observability hub and route note; block-monitoring note; assess and observability checklists; industry and comparison references; `llms.txt`; `CHANGELOG.md` | The p99 limitation now names all three Sum-only volume operation-time/count pairs. Amazon S3 documents Access Point-filtered request metrics, but whether that configuration applies to an Access Point attached to an FSx for ONTAP volume remains `open`. CloudTrail S3 data-event logging is documented separately. |
