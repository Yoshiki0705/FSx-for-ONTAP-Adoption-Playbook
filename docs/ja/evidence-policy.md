...
| `documented` | ベンダーまたは AWS の公式ドキュメントに記載がある。**または、測定環境を持つ別プロジェクトの記録に記載がある**（下記） | `source`（URL または文書名） | 一次情報として扱える。ただし版・リージョン差に注意 |
...
区分は **その記述の出どころ** を分類します。**追跡の度合いや、確実さの段階ではありません。** 別の語彙を使うリポジトリと相互リンクするとき、この境界で意味がずれます。

### `documented` が実測を含意しないこと

`documented` が意味するのは「ベンダーまたは AWS のドキュメントに記載がある」ことだけです。**著者がその挙動を自分で確認したという主張は含みません。** 実測を主張する区分は `verified` だけです。

したがって「一次資料には記載があるが、実機では追っていない」は `documented` です。**この対応で情報は落ちません。** 別のリポジトリが同じ状態を `unverified` のような語で呼んでいる場合も、そのまま `documented` に写せます。

**他のプロジェクトが測った数値を転記した場合も `documented` です。** `verified` が主張するのは「このリポジトリの著者がその環境で再現した」ことなので、引用はそれを満たしません。**ただし引用には固有の条件が 3 つ付きます。**

| 条件 | 理由 |
|---|---|
| **測定条件を全項目転記する** | 条件を外した数値は設計に使えません。世代・容量・IOPS・キャッシュ状態・クライアントの型・並列度まで |
| **引用元が未測定としている範囲も転記する** | **引用は都合のよい部分だけを取り出せます。** 引用元が挙げている空白を落とすと、確実さを偽ることになります |
| **[プロジェクト間の引用索引](reference/cross-repo-index.md) に登録する** | **引用は静かに腐ります。** 引用元が主張を撤回してもこちらの記述は変わりません。登録すると `make cross-repo-external` が撤回を検出します |

**実際に撤回は起きています。** 単一接続の測定について、引用元が「3 構成すべてが EC2 の 1 フロー上限」という説明を訂正し、そのうち 1 つは別の上限だったと記録し直しました。**ゲートが検出したのでこちら側も直せました。** 登録していなければ、誤った記述が残ったままになっていました。

**そして、測定にかける費用や時間が足りなかったことは区分を決めません。決めるのは出どころです。** 一次資料があれば `documented`、別プロジェクトの実測を転記したなら `documented`（上記 3 条件付き）、**どちらも無く推論で書いたなら `hypothesis`** です。**「予算を超えたから `documented`」は誤りです。** 裏づけが無いまま `documented` にすると、読者は一次資料があると読みます。

### 区分ではないドキュメントの不在

「探したが公開資料に記載を見つけられなかった」は、**製品の挙動についての主張ではなく、資料の状態についての主張です。** 4 つの区分はいずれも記述の裏づけを分類するもので、裏づけの不在を表しません。

とくに `hypothesis` は使わないでください。`hypothesis` は「論理的に導いた推論がある」ことを意味します。**推論を持たないまま使うと、持っていない根拠を持っているように見えます。**

書く場所は本文です。**探索日と探索範囲を明記してください。**「2026-08 時点では AWS の公式ドキュメントに記載を見つけられませんでした」のように、いつ・どこを探したかが分かる形にします。上限値やクォータであれば [上限値・クォータ](reference/limits/) の「実測できなかった項目」節が置き場です。
