...
|---|---|---|
| **「コピーを増やさずに使いたい」という要件** | [S3 Access Point は全リクエストを 1 つの ID で認可する](notes/reaching-data-without-copies.md) | コピーを増やさない 3 つの手段と、**権限が平坦化されること** |
| **S3 API で使うという前提** | [「S3 として使える」わけではない](notes/s3-access-point-constraints.md) | S3 との差分。設計段階で効く制約 |
| **学習データや実験の版管理** | [学習データセットの版をスケジュール Snapshot に載せると消える](notes/dataset-versions-and-experiment-branches.md) | 容量ではなく**ボリューム数の上限**に当たること |

**AI / RAG で権限を効かせたいなら、1 行目を読んでから実装リポジトリへ進んでください。** 判定を置ける場所は 3 つあり、[どこに置くか](notes/reaching-data-without-copies.md#ai--rag-で設計する対象)で境界の位置が変わります。
...
| [Agentic RAG](https://github.com/Yoshiki0705/FSx-for-ONTAP-Agentic-Access-Aware-RAG) | 権限を意識した RAG。**元の ACL が引き継がれるのではなく、権限を別の索引に再構成して検索時に判定**します（[平坦化の意味](notes/reaching-data-without-copies.md#権限が平坦化されることの意味)を読んでから） | CDK + Bedrock |
