...
lang: ja
---
# 複製先のデータは SnapMirror を止めずに S3 API で読めるか？

読めます。判定は junction path の有無で、宛先を mount すれば AP を取り付けられます。

## このノートで学べること

- SnapMirror 宛先を break せずに S3 Access Point で読めること（判定は junction path の有無で RW/DP ではない）
- `UpdateVolume` が DP ボリュームへの junction path 指定を 200 で無言破棄し、`DescribeVolumes` で読み直す必要があること

## このノートが答えないこと

- Athena 以外のエンジン・クロスリージョン・大規模での挙動（未測定、推論は「検証済み」として出さない）
- FlexCache の Cache Volume への取り付け（種別で拒否。ONTAP 版でも解決しない）
...
<a id="snapmirror-の宛先は-break-せずに-s3-api-で読める"></a>
...
| # | 手順 | 確認できること |
|---|---|---|
| 1 | 宛先ボリュームに対して `aws fsx update-volume` で junction path を指定し、直後に `aws fsx describe-volumes` で `JunctionPath` を読む | 200 が返っても値が `null` のままであること。**応答を成功の証拠にしてはいけない理由** |
...
| 4 | 反映後に S3 Access Point を取り付ける | 取り付けが成立すること。反映前は `the volume is not mounted` で失敗します |
| 5 | Access Point 経由で `ListObjectsV2` と `GetObject`、続けて書き込みを試す | 読めること、書き込みが拒否されること |
| 6 | ソースに書き込んで転送を起こし、同じ Access Point で再読み取り | 転送が Access Point の変更なしに反映されること |
| 7 | `snapmirror show` の `state` と `healthy` を通しで確認 | 関係が稼働し続けていること |

適用手順の全体像は [本番に取り入れる前の確認](../../../evidence-policy.md#本番に取り入れる前の確認) を参照してください。

手順 1 の判定は、junction path を読み直す次の読み取り専用コマンドで行います。

```bash
aws fsx describe-volumes --volume-ids <vol-id> \
  --query 'Volumes[0].OntapConfiguration.JunctionPath'
```

### 期待結果

```text
mount 済みなら junction path（例 /x）が返る。null なら未 mount で、S3 Access Point を取り付けられない。
UpdateVolume が 200 を返しても DP ボリュームでは値が反映されないため、ここを読み直して判定する
```

このコマンドはボリュームの junction path を読むだけで、ボリュームにも SnapMirror 関係にも変更を加えません。junction path の設定自体は ONTAP 側の操作です。

## Read next

[ファイルの到着をイベントとして受け取れるか？](fpolicy-fits-by-how-writes-land.md)
