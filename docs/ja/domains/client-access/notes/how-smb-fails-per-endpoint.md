...
# SMB の失敗は端末ごとに違う症状で出るか？

出ます。同じ原因が端末ごとに別の文言になるので、症状から原因は逆引きできません。

## このノートで学べること

- 同じ原因（名前解決・経路・資格情報・共有名・バージョン交渉）が Windows / macOS / WSL2 で別の症状になること
- 症状から逆引きせず確認の順序を固定すること、端末を疑い切った後に残るストレージ側の原因

## このノートが答えないこと

- 表のエラー文言の自環境 3 端末での実測値（未測定、想定される対応として読む）
- NTFS ACL や監査要件そのものの適合判断
...
**症状から入らず、名前解決 → 経路 → IP マウント → 共有名 → 資格情報 → バージョン交渉の順で潰します。** 本文の「確認する順序」がその全体像です。

| # | 手順 | 確認できること |
|---|---|---|
| 1 | `nslookup <svm-dns-name>` | 名前解決の可否。落ちたら Client VPN の DNS 設定 |
| 2 | SMB ポート 445 への到達性を試す | 経路かセキュリティグループ。NFS 併用なら 111 / 635 / 4045-4046 も別に確認 |
| 3 | IP でマウントする | 通ったら原因は名前解決で、経路と資格情報は正常 |
| 4 | 全端末で一様に失敗するか確認する | 一様ならストレージ側（SMB 不提供 SVM / 監査宛先枯渇）を先に見る |

**手順 3 が判定の中心です。** IP で通ったら「経路は正常」であって「設定は正常」ではありません。

手順 2 のポート到達性は、次の読み取り専用コマンドで確認できます。
...
```text
接続できれば経路とセキュリティグループは通っている（原因は名前解決・共有名・資格情報・
バージョン交渉のいずれか）。長く待って失敗するなら経路かセキュリティグループ側
```

このコマンドはポートへの到達性を試すだけで、端末にもファイルシステムにも変更を加えません。

## Read next

[端末から S3 Access Points に届くには何が要るか？](what-an-endpoint-needs-to-reach-s3-access-points.md)
