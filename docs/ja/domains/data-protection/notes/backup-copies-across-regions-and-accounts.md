...
**この値を監視や自動化の判定に使う場合、復元中に読むと `DP` を見ます。** DP ボリュームはバックアップ対象外なので、「復元直後に自動でバックアップを取る」処理を素朴に組むと、判定が状態に依存して揺れます。
...
| `StorageEfficiencyEnabled` | `false` | `true` | **引き継がれませんでした** |
| `SecurityStyle` | `UNIX` | API 応答では空 | NFS 経由のパーミッション動作は UNIX として正しく、フィールドのみ空。**一度のみの観測で、再現確認は未実施** |
| `TieringPolicy` | `NONE` | `NONE` | 復元時に指定した値 |

**ストレージ効率は復元後に再確認してください。** 容量前提を引き継いだつもりで復元すると、消費量が想定と変わります。
...
| 復元すればソースと同じ属性で戻る | `StorageEfficiencyEnabled` は引き継がれませんでした（実測） |
...
| 復元中の `OntapVolumeType` が `DP` になること、`StorageEfficiencyEnabled` が引き継がれないこと | **実測**（同上） |
| `SecurityStyle` が API 応答で空になること、FlexGroup のバックアップ作成が非同期で失敗すること | **一度のみの観測。再現確認は未実施。一般化しません** |
