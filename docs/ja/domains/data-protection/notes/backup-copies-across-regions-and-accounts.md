...
| スタンバイ系からアクティブ系への切り戻し経路 | **バックアップコピーには戻す経路がありません。** 復旧側で運用を継続するか、改めて逆向きにコピーします | 関係を削除して DR 側から `snapmirror resync` で逆向きに再同期（`documented`） |
...
**スタンバイ系からアクティブ系へデータを切り戻せるかどうかが、最も大きな差です。** SnapMirror は関係を逆向きに張り直して切り戻せます。バックアップコピーはリストアしかなく、できるのは新しいボリュームです。切り戻しを手順として持ちたい場合、バックアップコピーはその代替になりません。
...
| 要素 | 効き方 |
|---|---|
| **容量プール Standard の単価は、バックアップストレージの単価より低い** | 常時起動側の GB あたりが安くなる。ただし **Single-AZ に限った差で、Multi-AZ ではほぼ同額になります** |
| **バックアップは世代数ぶん積み上がる** | 保持世代 × 変更率のぶん、課金対象が論理データ量を超える |
| **確保課金に最小値がある**（SSD 容量・スループット容量） | 常時起動側に固定費としての最小金額ができる。**`All` 階層化にしても最小金額の考慮は必要で、メタデータが SSD に残るため最小 SSD 容量は確保したままです** |

固定費としての最小金額を、GB あたりの差で割った量が逆転点です。**それを下回る規模ではバックアップコピーが安く、上回る規模では常時起動が安くなります。** どちらの側でも RPO / RTO と切り戻し経路は上表のままなので、**規模が大きい環境では「RPO 分単位を選ぶと高くなる」という前提が成り立ちません。**

**逆転点は確保したスループットで動きます。** 最小金額のうちスループットが占める分が大きいため、**待機系に本番相当のスループットを積むと逆転点は上がり、最小に寄せると下がります。** 「常時起動は高い / 安い」を単独で言えないのはこの依存があるためで、比較するときは**データ量とスループットの組み合わせ**で並べてください。

最小金額を決める最小値は次のとおりです（`documented`、[Quotas](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/limits.html)）。

| 制約 | 値 |
|---|---|
| 最小 SSD 容量 | 1,024 GiB / HA ペア |
| 最小スループット容量（第 1 世代） | 128 MBps |
| 最小スループット容量（第 2 世代・1 HA ペア） | **384 MBps** |

**第 2 世代は最小スループットが 3 倍で、単価も第 1 世代より高いため、同じ「最小構成」でも最小金額が大きく変わります。**
...
| 前提 | 効き方 |
|---|---|
| **複数の同時リストアを合わせた値** | 単発 1 本がこのレートで走るとは書かれていません |
| **バックアップとリストアは背景処理で、未使用のスループット容量しか使わない** | **スループット容量がそのまま上限になります。** 128 MBps のファイルシステムで 250 MBps は出ません |

したがって `min(公開レート, スループット容量)` が上限で、**クライアント I/O がある時間帯はさらに遅くなります。** 数 TB を超えると時間単位、数十 TB では日単位になります。**小さいファイル中心のデータセットは 100 MBps が先に効くため、スループットを上げても縮みません。**

> **10 TB 以上の実測値は公開情報の中に見つけられませんでした**（`unverified`）。AWS ブログ、re:Post、ナレッジセンター、公式ドキュメントを対象に探し、**容量と所要時間の両方を明記した実測の公開例はありませんでした**（2026-08-29 時点）。所要時間が RTO に効く環境では、**本番相当の容量とファイル構成で自分で測るしかありません。**

**この所要時間が「待機系は第 1 世代で足りる」を覆す場合があります。** 第 2 世代は**リストア中でも読み取りできます**（メタデータのロード後）。メタデータはバックアップデータの **1〜7%**（小さいファイルほど多い）と記載されているため、**大容量でも先にマウントして読み始められます。** 第 1 世代はリストア完了まで読めません（`documented`）。

| データ量と RTO | 選択 |
|---|---|
| 数 TB 以下、または RTO が日単位 | 第 1 世代（最小金額が安い） |
...
> [AWS Backup 料金ページ](https://aws.amazon.com/backup/pricing/) で現在値を確認してください。
