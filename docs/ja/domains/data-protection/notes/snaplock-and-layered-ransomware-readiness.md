...
# Amazon FSx for NetApp ONTAP の SnapLock は有効化だけで保護できるか？

SnapLock は保持期間と WORM 移行で保護します。監査ログ・Snapshot ロックは削除不能範囲を広げます。

## このノートで学べること

- SnapLock の有効化、WORM への移行、保持モード、特権削除の違い
- 監査ログボリュームと Snapshot locking が削除可能性と影響範囲に与える制約

## このノートが答えないこと

- 法令・規制への適合性の判断
- 保持期間、Compliance、特権削除、監査ログ、Snapshot locking を有効化する手順
...
- [保存時の暗号化は自動、転送時は方式ごとに条件が異なる](../../security-governance/notes/what-the-platform-gives-and-what-stays-yours.md) — 監査と権限の分離
...
次のコマンドは、既存ボリュームの SnapLock 構成を読み取ります。

```bash
aws fsx describe-volumes --volume-ids <volume-id> --query 'Volumes[].OntapConfiguration.SnaplockConfiguration' --output json
```

### 期待結果

```json
[<snaplock-configuration-object-or-null>]
```

**確認できる範囲には境界があります。** この読み取りは SnapLock を有効化せず、保持期間を設定せず、監査ログボリュームを作成せず、WORM へコミットせず、削除を試さず、コンプライアンスクロックを初期化せず、削除可能性を証明しません。`null` または構成オブジェクトのどちらが返っても、ロック済みファイルやロック済み Snapshot の有無は分かりません。
...
**上の読み取りとは別物です。** 次の表は不可逆な操作を含み、上のコマンドの続きではありません。使い捨ての検証環境でのみ、順に実行してください。
...
[第三者バックアップ製品が届く経路は AWS の API 側ではない](third-party-backup-reaches-it-by-another-route.md)
