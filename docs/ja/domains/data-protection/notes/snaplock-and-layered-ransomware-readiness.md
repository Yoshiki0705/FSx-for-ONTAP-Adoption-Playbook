...
| 検知入力 | 閾値の形 |
|---|---|
| 改名の操作レート | **学習した基準に対する 100% のサージ** |
| 削除の操作レート | 同じく 100% のサージ |
| 未知の拡張子 | **48 時間内に 5 種類**（既定値） |
...
| b | 1,000 件を一括削除 | 検知なし |
| c | **未知の拡張子 7 種類**に書き換え（内容不変、閾値 5 を超過） | 検知なし |

**対照群が検知されたので、これは「ARP が動いていなかった」ではありません。** そして c は閾値を
超えていながら検知されておらず、**ARP は 7 種類すべてを観測済みの拡張子として記録していました。**
**観測されていることと、攻撃と判定されることは別です。**
...
>
> **未測定の点を 1 つ明示します。** サージ判定は学習した基準との比較なので、**基準が形成されていない
> 新規ボリュームでは判定が成立しない可能性があります。** 基準が育ったあとの同じ操作は測っていません。
> **これは非検知の理由の候補であって、確認した事実ではありません。**
