...
**3 行目の読み方を一度誤りました。** 「48 時間内に 5 *種類*」ではありません。[CLI のリファレンス](https://docs.netapp.com/us-en/ontap-cli-9171/security-anti-ransomware-volume-attack-detection-parameters-show.html)は
...
超えていますが、**継続時間の条件（48 時間）を満たしていません。** 操作は数秒で終わり、観測は 30 分です。
**30 分の観測で 48 時間の条件は試せません。** c が示したのは「閾値超過だけでは判定されない」ではなく、
**この観測設計ではこの入力を試せていない**ということです。
...
> | 未知拡張子の継続時間 | 条件は **48 時間の継続**。観測は 30 分でした |
>
> **どちらも「文書に沿った説明」であって、推測ではありません。** そして**確認はされていません。**
> **追試の設計はこれで決まります — 履歴を作ってから測る。** 新規ボリュームで測ると、機構の性質ではなく
> **履歴の不在を測ることになります。** 経路も NFSv3 の 1 経路だけで、SMB と S3 Access Point は未測定です。
