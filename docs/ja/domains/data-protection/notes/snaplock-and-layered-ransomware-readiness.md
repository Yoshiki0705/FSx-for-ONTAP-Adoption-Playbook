...
**そして、パラメータで切り替えられない検知経路が 1 つあります。** 同じリファレンスは
「**拡張子とエントロピーを組み合わせた検知は常に有効で、このパラメータに依存しない**」と記載しています。
**つまり上の表は切り替え可能な入力の一覧であって、検知入力の全体ではありません。**
...
**S3 Access Point 経由の操作が、改名・削除のカウンタに数えられるかは分かっていません。** ボリューム自体は
NAS ボリュームなのでパラメータは有効ですが、**カウンタが S3 経路の操作を拾うかは別の問題**です。
