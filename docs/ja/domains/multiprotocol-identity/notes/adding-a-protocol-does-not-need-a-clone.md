...
| 4 | **name-mapping が成立しているか** | マッピングを設定します。**NTFS スタイルのボリュームでは、権限評価に NTFS ACL を使うため Windows→UNIX マッピングが参照されません**（[セキュリティスタイルと権限評価の対応](security-style-and-permission-evaluation.md#セキュリティスタイルと権限評価の対応)） | 不要 |
