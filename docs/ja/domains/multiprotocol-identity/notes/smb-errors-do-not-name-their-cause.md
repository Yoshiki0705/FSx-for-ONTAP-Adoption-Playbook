...
名指しません。資格情報の誤りに見えるものが、実はアカウント不在や共有不在であることがあります。
...
- SMB クライアントの汎用エラーが原因を一意に指さず、アカウント不在・共有不在・UNC 破損に対応すること
- SMB で到達できるのは共有だけで、ボリュームの junction path は共有名ではないこと

## このノートが答えないこと

- 同じエラー文字列が別の原因でも出るか（未確認）
- Windows のバージョン差
...
共有の一覧は、次の読み取り専用コマンドで確認できます。

```bash
curl -X GET -u fsxadmin -k "https://<管理エンドポイント>/api/protocols/cifs/shares?fields=name,path,svm.name"
```

### 期待結果

```text
データ用の共有が存在するか（c$ / ipc$ しかない状態を「共有あり」と数えない）
```

この確認で分かるのは構成済みの共有の一覧だけです。アカウントの存在、445 の開閉、マウントの成否は別に確認します。何も作成・変更しません。
