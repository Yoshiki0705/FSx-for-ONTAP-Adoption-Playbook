...
セッションの終わらせ方だけを変えて 5 通り測りました。**決めているのは「共有のマッピングを外したか」ではなく「SMB セッションそのものが破棄されたか」です。**
...
> 4634 から組み立てないでください。**測った 6 経路のうち 4 経路が無音です** — マッピング解除
> 2 種はその時点で出ず、通信断による自然回収と管理者切断は最後まで出ません。
...
- **アイドルのみでのセッション終了**。今回測ったのは通信断による孤立セッションの回収（約 3 分）です。**通信が生きたままアイドルで放置した場合にセッションが終了するか、その際に 4634 が出るかは測っていません**
