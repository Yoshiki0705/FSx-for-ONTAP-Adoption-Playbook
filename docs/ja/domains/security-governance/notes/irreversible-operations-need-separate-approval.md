...
削除できなくする機能は実行者判断で有効にせず、保持値を含めて明示指示を受けてから実行します。

## このノートで学べること

- 削除する能力を取り除く機能は、保持値を含めて明示指示を受けてから実行すべきこと
- 影響範囲が呼び出しで指定したリソースを超えて SVM やファイルシステムに及ぶこと

## このノートが答えないこと

- 監査ログの保持期間より短い値が実際に拒否されるか（試すと同じロックが増える）
- サポート経由での早期解除（できないと確定済み）
...
既存ボリュームの SnapLock 構成は、次の読み取り専用コマンドで確認できます。
...
この読み取りは何も作成・有効化・ロック・削除しません。返る値からは、ロック済みファイルの有無も、監査ログボリュームの保持期間も、削除可能性も分かりません。不可逆な操作の検証は使い捨ての専用ファイルシステムでのみ行ってください。
