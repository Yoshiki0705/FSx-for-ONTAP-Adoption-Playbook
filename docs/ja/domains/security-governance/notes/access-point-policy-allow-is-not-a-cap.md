...
そのうえで、設計を誤らせる挙動が 1 つあります。**同一アカウント内では、アクセスポイントポリシーに書いた `Allow` は権限の上限になりません。**

| 測定した状況 | 結果 |
|---|---|
| ポリシーを 1 つも付けていない AP に、S3 権限を持つ同一アカウントの IAM ユーザーがアクセス | **成功します。** AP ポリシーは必須ではありません |
| ロールだけを `Allow` したポリシーを付け、そこに書いていない IAM ユーザーがアクセス | **成功します。** 名前を書いていない主体を排除しません |
| `Allow` の `Action` に `s3:PutObject` を書いていない状態で、ロールが `PutObject` | **成功します。** 操作の種類も絞れません |

同一アカウントの評価は identity-based ポリシーと resource-based ポリシーの **和** です。だから **絞りたいなら明示的な `Deny` を書く**ことになります。

**その `Deny` の書き方に落とし穴があります。** `NotPrincipal` で例外を作る形は、**除外したはずの主体まで拒否しました。** 推奨は `Condition` の `StringNotEquals` に `aws:PrincipalArn` を使う形です。
...
**「AP ポリシーに書いた範囲しか通らない」という前提で設計すると、実際には広く通ります。**

| ポリシー | 呼び出し元 | 操作 | 結果 |
|---|---|---|---|
| なし | IAM ユーザー | `GetObject` | 成功 |
| なし | IAM ユーザー | `ListObjectsV2` | 成功 |
| ロールのみ `Allow`（`GetObject`, `ListBucket`） | IAM ユーザー（**未記載**） | `GetObject` | **成功** |
| 同上 | ロール | `GetObject` | 成功 |
| 同上 | ロール | `PutObject`（**`Action` 未記載**） | **成功** |
