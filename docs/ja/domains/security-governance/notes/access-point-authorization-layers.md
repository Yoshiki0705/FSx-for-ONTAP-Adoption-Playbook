...
限れば ARP がこの経路を見ている（実測: アクセスポイント経由で書いた高エントロピーオブジェクトが
ARP の suspect として記録された。2026-08-26 / ONTAP 9.18.1P3D1 / ARP/AI。**件数と両経路の対照は
検証記録にある** — ファイルプロトコル側も同時に検知され、**経路による差は観測されていない**）。
**観測された検知理由は高エントロピーのみである。** 改名・削除のレートも検知入力に含まれるが、
