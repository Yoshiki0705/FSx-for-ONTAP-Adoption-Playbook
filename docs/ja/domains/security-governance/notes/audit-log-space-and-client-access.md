...
**そして停止の要因は、ステージングではなく宛先の枯渇でした。** 本検証で埋めたのは宛先ボリュームで、埋まった時点でクライアントアクセスが止まりました。NetApp の KB [監査ログのデスティネーションがフルのため、CIFS 共有でデータが提供されていません](https://kb-ja.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/CIFS_share_not_serving_data_because_the_Audit_Log_Destination_is_full) が扱っているのも、宛先の枯渇による同じ症状です。**したがって「2 GB のステージングに遠く及ばないのに停止した」ことは矛盾ではありません。ステージングは関係していませんでした。**
...
**この 0 件は何の証拠にもなりません。** `fsxadmin` から `adt.*` のイベントが 1 件も見えないという事実だけがあり、**「発火しなかった」のか「発火したが利用者からは見えない」のかを、当方では切り分けられません。** ステージングボリューム `MDV_AUD_*` を意図的に埋める手段が `fsxadmin` に無いため、発火する条件を作って可視性を確かめる試験もできません。**区別できない 2 つの状態のうち一方を選んで根拠にしていたのが誤りでした。** 結論そのもの（ステージングは関係しない）は上記の宛先枯渇の実測と KB で立ちますが、**当時それを支えていた論拠は成立していませんでした。** イベントの可視性は `open` として扱います。

一方、`MDV_aud_*` を対象とした `monitor.volume.full` / `monitor.volume.nearlyFull` も 2 回を通じて 0 件でした。**こちらも同じ理由で読めません。** これらが `fsxadmin` から参照できるかを当方で確認できておらず、ステージングを意図的に埋める手段も無いためです。**したがってこの 0 件を「ステージングは 95% に達していなかった」と読むこともできません。**
...
| 自動拡張（ONTAP CLI の `volume autosize`） | 併せて検討 | **19 秒では人手もアラーム経由の自動化も間に合いません。** 使用率のしきい値で自動的にボリュームを拡張します。[Enabling autosizing](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/enable-volume-autosizing.html) が、FSx for ONTAP の既定のボリューム種別である FlexVol に対して ONTAP CLI の `volume autosize` を使う手順として記載しています |
