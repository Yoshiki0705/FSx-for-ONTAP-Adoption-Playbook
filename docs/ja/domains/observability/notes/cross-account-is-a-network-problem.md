...
# 別アカウントの監視は IAM ロールで済むか？

済みません。接続先は ONTAP の管理エンドポイントで、ネットワーク到達性の問題です。

## このノートで学べること

- クロスアカウント監視が IAM ではなくネットワーク到達性（Transit Gateway・双方向ルート・CIDR 許可）の問題であること
- 収集元の数を決めるのは拠点の境界ではなく接続の向き（pull は 1 か所、push だけ拠点ごと）であること

## このノートが答えないこと

- 拠点をまたぐ実配置で実際にどうなるか（未測定）
- push 経路（FPolicy）の再登録運用の効果や負荷の実測値
...
<a id="クロスアカウントは-iam-ではなくネットワークの問題"></a>
...
**IP で登録されることは、拠点数のほかに再登録の運用も呼びます。** タスクが置き換わるたびに登録が
古くなり、その間のイベントが失われます。運用要件と本番構成の選択肢は
[FPolicy が適合するかは、データをどう読むかではなく、どう書くかで決まる](../../data-utilization/notes/fpolicy-fits-by-how-writes-land.md)
にあります（**別プロジェクトの実測の転記を含むため、データ活用ドメインに置いています**）。
...
手順 1 の到達性は、監視側インスタンスから次の読み取り専用アクセスで確認できます。

```bash
curl -sk --connect-timeout 5 https://<management-endpoint>/api/cluster?fields=version
```

### 期待結果

```text
到達できればクラスタのバージョン情報が返る。無言でタイムアウトする場合は
ネットワーク到達性（双方向ルート・CIDR 許可）の側であって、IAM ではない
```

このアクセスは ONTAP 管理エンドポイントの到達性を確認するだけで、ルートやセキュリティグループの設定は何も変更しません。

## Read next

[監視経路は認証とアクセス経路でどこまで狭まるか？](route-choice-is-bounded-by-access-and-auth.md)
