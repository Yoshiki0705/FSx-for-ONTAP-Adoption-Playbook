...
出典のない手書きの一覧で、取り込み先のフォルダを作るスクリプトまで存在していました。**
どちらも訂正されています。

**ただし範囲が 1 つあります。** メトリクスの集合は**自分で publish すれば増やせます** — あちらは
ONTAP REST API から取得した値を独自の名前空間で publish しており、**その分についてはダッシュボードが
移せます。** つまりこの主張は「**マネージドサービスが公開するメトリクスの集合では**移らない」であって、
「移す方法が無い」ではありません。**移したいダッシュボードが依存しているメトリクスを先に数えてください。**
