...
- [ ] **`multipath -ll` のパス数を数え、その本数を選んだ理由を書いた。** **セッション数は既定でも上限でもありません。** Linux では 8 セッションの手順が `(Optional)` で、適用しなければ 2 パス、適用すると 16 パスです。Windows は公開スクリプトの既定が 8 なので 16 パスになります
- [ ] **1 ノードあたりのパス数が 8 を超えていないことを確認した。** ONTAP のプラットフォーム非依存の上限です
