...
- ブロックのデータを S3 API で分析するには 4 段を経由し、コピーが 1 本発生すること

## このノートが答えないこと

- 「LUN の中身はファイルプロトコルから読めない」を明示した公開一次情報（見つけられず `open`、実測で確認可能）
- crash-consistent なクローンからのコピーを監査・照合に使ってよいかの判断
...
**手順 4 を省くと、何も見えなかった結果が「LUN だから見えない」なのか「export policy とアクセスポイントの設定が間違っている」なのか分かりません。** [コントロールのない失敗は対象の性質ではなく手順の誤りを記録しうる](../../../evidence-policy.md)ためです。記録する項目は ONTAP バージョン、ボリュームのセキュリティスタイルと `os_type`、手順 4 の対照が成功したことです。

手順 2 の一覧は、LUN を含むボリュームを NFS マウントした先で次の読み取り専用コマンドを実行します。
...
```text
LUN はファイルとして現れない（対照で置いた普通のファイルは現れる）。
現れないことが LUN の性質だと言えるのは、対照の普通のファイルが同じ経路で見えたときだけ
```

このコマンドはマウント先を一覧するだけで、LUN にもボリュームにも変更を加えません。同じデータはブロック経由（iSCSI マウント）でのみ読めます。

## Read next

[ブロックプロトコルの選択肢は世代で先に狭まるか？](protocol-choice-is-bounded-before-you-choose.md)
