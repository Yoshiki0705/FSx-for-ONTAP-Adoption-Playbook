...
| 複数の LUN を別ボリュームに置いても Snapshot は同時刻 | **個別に取れば時刻が違います。** consistency group の write fence を使うと同一時刻になります |
| 別ボリュームにまたがる DB は crash-consistent では復旧できない | **write fence 付きの consistency group Snapshot から復旧しました。** コミット済みの行は残りました |
| クローンのマウントには常に `-o nouuid` が必要 | **親を同じホストにマウントしているときだけです。** 別ホストではそのままマウントできました |
...
| クローンは元と同じホストにそのままマウントできる | **XFS では `-o nouuid` が必要でした**（UUID が親と一致するため） |
...
| snapshot 予約 0%、snapshot autodelete という構成例 | [AWS: Best practice configuration of Amazon FSx for NetApp ONTAP for Microsoft SQL Server workloads](https://aws.amazon.com/blogs/storage/best-practice-configuration-of-amazon-fsx-for-netapp-ontap-for-microsoft-sql-server-workloads) |
