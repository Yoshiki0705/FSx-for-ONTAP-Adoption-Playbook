...
| **除数が実測と一致するか** | **どちらのプロトコル群でも一致しません。** ファイルは除数を下回り、**ブロックは 1.82 倍上回りました**（下記） |
...
| 実測（1 MiB 逐次読み、数えた TCP 1 本） | 625 MBps に対して |
|---|---|
| iSCSI 単一フロー 1,135.18 MB/s（= 9.08 Gbps） | **181.6%** |
...
**なぜ約 2 倍になるのかは引用元も確認していません。** 候補は複数挙がっていますが（世代の新しい
インスタンスの 1 フロー上限、5 Gbps という記述の適用条件）、**どれが当たっているかは検証されて
いないので、原因を前提にした設計をしないでください。**

**1 接続でのプロトコル差は再現しませんでした。** iSCSI と NVMe/TCP を同一ファイルシステム上で
測り直した結果が 0.06% 差で、**プロトコルの選択は単一接続のスループットを動かしません。**
選ぶ理由は別の軸に置いてください。
