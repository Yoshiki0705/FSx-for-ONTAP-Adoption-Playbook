...
**未検証なのは除数ではなく、次の 2 つです。**

| 未検証な前提 | 状態 |
|---|---|
| **線形性** — n セッションが 1 セッションの n 倍になるか | **未測定。** ブロックでの実測は 1 点も存在しません |
| **除数が実測と一致するか** | **ファイルプロトコルでは一致しませんでした**（下記） |

**「AWS が 625 MBps と書いている」と「625 MBps で測れた」は別の主張です。** この節で言えるのは前者だけです。

### 除数と実測の差（ファイルプロトコルでの観測）
...
**ブロックが同じ位置に来るなら、625 で割る形は必要セッション数を約 5% 少なく出します。**
**そこで、割って出た数の `nr_sessions` に 1 を足してください（パスは 2 本増えます）。** 端数の切り上げでは
足りません——6.0 のように割り切れるときに余裕がゼロになります。

**単位を書いているのは、素の「1 本」が 2 通りに読めるためです。** このノートの上半分で確定させた
数え方がそのまま返ってきます。`nr_sessions` はノードあたりの値で、SVM は両ノードに iSCSI LIF を
持つので、**パスは偶数刻みしか動きません。** つまり「パスを 1 本足す」は `nr_sessions` では設定できず、
`nr_sessions` に 1 を足すと**パスは 2 本増えます。** 読者が実際に打つのは `nr_sessions` の値なので、
指示はその単位で書いています。

**ただしこれはファイルプロトコルの観測です。** ブロックでの実測は存在しないので、
**591.62 をブロックの見積りに流用しないでください。** 言えるのは「ファイルプロトコルでは 5〜8% 低かった。
ブロックは未測定」までです。

> **Evidence**: 除数は `documented`（上記 2 ページ、2026-09-07）。**5〜8% の差はファイルプロトコルの
> 実測の転記**で、[引用元の測定結果](https://github.com/Yoshiki0705/S3-Burst-on-ONTAP-Files/blob/main/docs/ja/verification/perf-matrix-results.md)に基づきます
> （測定条件は引用元にあります）。**ブロックプロトコルのスループット値は、このリポジトリにも引用元にも
> 1 点も存在しません。**
