...
- 最小構成の月額の 83% がスループット容量で、複製数がデータセット規模ごとの点を超えると総額が逆転すること

## このノートが答えないこと

- 現在の単価や自リージョンの月額（価格は改定される。設計前に引き直す）
- 重複排除・圧縮の削減率（データ依存で保証されない）や金額に出ない差の換算値
...
| # | 手順 | 得られるもの |
|---|---|---|
| 1 | Price List API で自リージョンの SSD とスループット容量の単価を引く | 床の計算に必要な 2 つの単価。**エンドポイントは `us-east-1` と `ap-south-1` だけです** |
...
手順 1 の単価は、次の読み取り専用コマンドで引き直します（Price List API のエンドポイントは `us-east-1` / `ap-south-1` のみ）。
...
```text
SSD とスループット容量の単価が返る。unit（MiBps-Mo など）と description の単位を突き合わせてから
計算する（pricePerUnit だけを引くと 1,024 倍ずれる項目がある）。床の 8 割はスループット容量
```

このコマンドは公開単価を読むだけで、料金にもリソースにも変更を加えません。数値は改定されるため、判断のたびに引き直してください。

## Read next

[LUN の中身はファイルプロトコルに現れるか？](lun-contents-do-not-reach-file-protocols.md)
