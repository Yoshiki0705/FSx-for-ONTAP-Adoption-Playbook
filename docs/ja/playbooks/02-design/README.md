...
|---|---|---|
| **まだ何も作っていない** | [デプロイタイプは一度しか決められない](notes/deployment-type-is-decided-once.md) | **不可逆な選択がどれで、いつ決まるか。** 作成後に変えられません |
| **既存の構成**（1 組の HA ペアで動いている） | [単一 HA ペアの天井](notes/deployment-type-is-decided-once.md#単一-ha-ペアの天井) | スケールアウトの上限。**可用性の選択がこの天井も決めています** |
| **利用者がどう届くかが未決** | [エンドユーザーがデータに届く経路は 4 つある](notes/how-end-users-reach-the-data.md) | 経路ごとに前提が違います。**ここを決めないとセキュリティスタイルも決まりません** |
...
| 7 | ファイルシステムと SVM をどの単位で分割するか | [分割の単位が決めているのは、後で決め直せない範囲](notes/the-split-decides-what-cannot-be-revisited.md) |
