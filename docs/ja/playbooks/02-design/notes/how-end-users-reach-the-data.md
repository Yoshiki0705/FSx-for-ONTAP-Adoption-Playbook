...
# エンドユーザーがデータに届く経路は何通りか？

4 つです。ファイル共有・ファイル転送・ブラウザ 2 種で、ブラウザだけマネージドで埋まっていません。

## このノートで学べること

- エンドユーザーがデータに届く 4 経路（NFS/SMB・SFTP 等・ブラウザ 2 種）と、それぞれマネージドで埋まる範囲が違うこと
- ブラウザ経路の認可が 3 層になり、3 層目（ファイルシステムユーザー）が見落とされやすいこと

## このノートが答えないこと

- ブラウザ × FSx for ONTAP にマネージドサービスが無いことの証明（確認できていないだけ、サービスは追加される）
- Transfer Family 経由の制約（rename / append / 5 GB）の自環境での再現
...
<a id="エンドユーザーがデータに届く経路は-4-つある"></a>
...
### Transfer Family 経路の制約 — 先に詰まるクライアント側
...
| S3 AP の作成時はファイルシステムと同一リージョン・同一アカウント | 他アカウントのボリュームには作成できません。作成済み AP のクロスアカウント利用は、AP ポリシーと呼び出し元の identity-based ポリシーの両方で許可します |
...
2 層目と 3 層目の評価順序、および症状からどちらで落ちたかを逆引きする手順は
...
> **セキュリティに関する補足**: AWS は Windows の `FileSystemIdentity` が参加済み Active Directory
> ドメインで解決できることと、名前サービスへ到達できない場合に Access Point が `MISCONFIGURED` に
> なりうることを記載しています。**すべてのデータ操作が常にドメインコントローラー到達性を必要とすることと、
> AD 到達不能時にも `HeadBucket` だけが成功することは、公開一次情報と完全な再現記録を確認できていないため
> `open` です。** 疎通確認では `HeadBucket` だけに依存せず、データ操作も確認してください。
...
**経路を決める前に、要件が 4 経路のどこに当たるかを写像します。** 特にブラウザ経路は認可が 3 層になるため、設計段階で層ごとの主体を決めておきます。

| # | 手順 | 確認できること |
|---|---|---|
| 1 | エンドユーザーが使う道具（マウント / SFTP クライアント / ブラウザ）を洗い出す | どの経路に当たるか |
| 2 | S3 AP を前提にする経路なら ONTAP バージョンが 9.17.1 以降か確認する | Transfer Family とブラウザ経路の前提 |
| 3 | ブラウザ経路なら、認可の 3 層（IdP・IAM/AP ポリシー・ファイルシステムユーザー）の主体を写像する | 3 層目の見落とし |
| 4 | Transfer Family を使うなら、実際のクライアントで rename を 1 度試す | 既定設定で失敗しないか |

**手順 3 が最も飛ばされます。** IdP のグループを増やしても 3 層目は増えません。1 つの S3 AP は 1 つのファイルシステムユーザーに紐づきます。

手順 2 の ONTAP バージョンは、次の読み取り専用コマンドで確認できます。
...
```text
version.generation.major が 9.17.1 以上なら S3 Access Points が作れ、Transfer Family とブラウザ経路の前提を満たす。
下回るならまずアップグレードの検討から。NFS / SMB 経路はバージョンに依存しない
```

このコマンドはクラスタのバージョンを読むだけで、経路にも認可にも変更を加えません。ブラウザ経路を自分で組む場合、認可の判定はアプリケーション側に残ります。

## Read next

[デプロイタイプは一度しか決められないか？](deployment-type-is-decided-once.md)
