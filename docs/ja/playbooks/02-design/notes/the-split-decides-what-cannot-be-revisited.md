...
後で決め直せない範囲です。ファイルシステム・SVM・ボリュームの境界ごとに固定されるものが違います。

## このノートで学べること

- 分割の単位が「何が独立に決め直せるか」を決め、境界ごとに固定される不可逆項目が違うこと
- SnapLock 監査ログと FlexCache のセキュリティスタイルは、境界を分けても独立にならないこと

## このノートが答えないこと

- どの分離パターン（マルチテナント等）が良いか（運用しておらず比較の根拠を持たない）
- FlexCache のセキュリティスタイル継承の当リポジトリでの再測定（sibling の実測を転記）
...
<a id="分割の単位が決めているのは後で決め直せない範囲"></a>
...
**4 で載っていたら、その境界を分ける理由があります。** 逆に、どの不可逆項目も安定した要件にしか載っていなければ、分けない理由があります。**5 を戻り値で済ませないでください。** 上の FlexCache の例のように、拒否が成功の形で返る操作があります。「エラーにならなかったから変えられる」と記録すると、不可逆項目を可逆と書くことになります。

現状の境界（デプロイタイプ・世代・HA ペア数）は、次の読み取り専用コマンドで確認できます。
...
```text
現在のデプロイタイプ・世代・HA ペア数が返る。これらはファイルシステム単位で固定され、変更できない。
Multi-AZ と Single-AZ の混在が要件なら、1 つのファイルシステムでは表現できず分割が要る
```

このコマンドはファイルシステムの構成を読むだけで、境界にもデータにも変更を加えません。不可逆項目の確認（手順 5）は、拒否が job の `state=failure` として返るため、値を読み直して判定してください。

## Read next

[エンドユーザーがデータに届く経路は何通りか？](how-end-users-reach-the-data.md)
