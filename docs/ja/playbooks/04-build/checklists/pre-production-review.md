...
- [ ] **特権削除を使うかどうかを決めた**。使う場合は同一 SVM に監査ログボリュームが必要で、**それを作った時点でボリューム・SVM・ファイルシステムが最低 6 か月削除できなくなります**（Enterprise モードでも例外なし）。使わない判断をすれば、このロックは発生しません。詳細は [不可逆な操作の承認は作業の承認とは別に取る](../../../domains/security-governance/notes/irreversible-operations-need-separate-approval.md)
- [ ] **Snapshot locking（Tamperproof Snapshot）を使うかどうかを決めた**。**SnapLock を使っていなくても有効にできます。** 有効化すると全ロック済み Snapshot の失効まで無効化できず、未期限のロック済み Snapshot があるボリュームは削除できません。**保持期間 × スケジュール頻度が 1 ボリューム 1,023 個を超えないことを計算**してください（ロック済み Snapshot には世代数の上限が適用されません）
...
- [ ] **FlexCache の write-back を使う場合、Origin の空き容量が 20% を割らない計画になっている**。**割ると無言で write-around に戻ります。** 閾値はボリュームの報告値と**アグリゲートの物理空き容量の両方**で評価されるので、Origin をオーバープロビジョニングしていると想定より早く切り替わります。**エラーは出ず、書き込み遅延が増えることで気づきます**（[write-back を選ぶと入れ替わる制約](../../../domains/data-utilization/notes/reaching-data-without-copies.md#write-back-を選ぶと入れ替わる制約)）
- [ ] **write-back を使う場合、ONTAP の版を層ごとではなく両側で決めた**。S3 Access Point が 9.17.1 以降、write-back は **9.17.1P1 以降を Origin と Cache の両方で強く推奨**（9.15.1 は本番向けに推奨されていません）。**収集層の要件だけで版を決めると、配布側で足りません**
- [ ] **write-back を使う場合、スナップショットの間隔と書き込みのピークをずらした**。Origin でのスナップショットは、その Origin に紐づく**すべての** write-back Cache からダーティデータを回収し、書き込みが多い時間帯では複数回の再試行が必要になります
...
- [ ] **S3 Access Point を使う場合、FPolicy 前提のリアルタイム制御に穴がないか確認した**。ランサムウェア検知・DLP・`mandatory` による遮断は AP 経由の書き込みに効きません。**ランサムウェアの検知は ARP が代替になりますが、遮断は代替になりません** — ベンダーの応答手順に書き込みを拒否する段がないためです。**遮断が要件なら AP ポリシーと IAM の側で設計してください**
- [ ] **ARP の学習期間を、ONTAP の版と*ボリューム種別*の 2 軸で確認した**。**「9.16.1 以降なら学習期間なし」は FlexVol に限った話です。** [ベンダーのドキュメント](https://docs.netapp.com/us-en/ontap/anti-ransomware/index.html)の対応表は、事前学習済みモデル（ARP/AI）が **FlexVol は 9.16.1 以降、FlexGroup は 9.18.1 以降、SAN は 9.17.1 以降**、旧モデル（30 日の学習期間）が **FlexVol 9.10.1〜9.15.1、FlexGroup 9.13.1〜9.17.1、SAN 非対応**としています（2026-09-07 に全文確認）。**つまり FlexGroup で 9.16.1 / 9.17.1 を使う環境は、版が 9.16.1 以降でも旧モデルで学習期間が必要です。** 30 日の学習モードは NAS FlexVol 向けで、**9.13.1 以降は自動切替が利用できます。** 版だけで判断すると、FlexGroup の読者に誤った計画を立てさせます
...
| SnapLock の保持モード（`COMPLIANCE` / `ENTERPRISE`） | 不可 | 変更するパラメータ自体がありません。作り直す |
| **SnapLock 監査ログボリュームの作成** | **不可。しかも影響がボリューム外に及びます** | **最低 6 か月、ボリューム・SVM・ファイルシステムのいずれも削除できません。** 待つ以外の経路がありません |
| `PrivilegedDelete=PERMANENTLY_DISABLED` | 不可（終端状態） | 再有効化できません。作り直す |
...
- [Snapshot があることと復旧できることは別](../../../domains/data-protection/notes/snapshots-are-not-a-recovery-plan.md#自環境での確認手順) — 復元を実際に試す手順
