...
> **所要時間は含みません。** 帯域とデータ量に依存するため、[自分の環境で測る](../notes/where-the-rollback-window-closes.md#自分の環境で確かめる)手順を参照してください。
...
- [ ] **ACL を移行元と比較した**。浅い階層ではなく、**継承の破れ・明示的な拒否・長いパス・実行アカウントが所有していないファイル**を選びます。手順は [自分の環境で確かめる](../notes/preserving-acls-during-migration.md#自分の環境で確かめる)
