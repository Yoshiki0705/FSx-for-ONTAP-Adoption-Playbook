...
# 容量が余っていても書けなくなるのはなぜか？

inode を使い切ると、空き容量があっても新しいファイルを作れなくなるためです。
...
- inode（ファイルポインタ）を使い切ると容量が余っていても書けなくなること、その症状の紛らわしさ
- 既定の inode 比率とドキュメント記載・実測の食い違い、自環境の `FilesCapacity` を読む必要性

## このノートが答えないこと

- inode を実際に使い切る破壊的試験の結果（読み取り専用の観測のみ）
- 最小サイズのボリュームまで外挿できる inode 比率（20 MiB では比率が一致しない）
...
| 記録する項目 | 理由 |
|---|---|
| 最大値と平均値、および公開される次元ごとの系列 | 平均は飽和を隠し、最大値だけでは継続性が分かりません。理由は [監視は平均値で失敗する](../../05-operate/notes/monitoring-fails-on-averages.md) にあります |
...
graph TD
    ...
    B --> C[総ファイル数と平均ファイルサイズを数える]
    C --> D{平均ファイルサイズは<br/>前表の inode 分岐点を下回るか}
    D -->|分岐点未満| E[inode 引き上げ<br/>または ボリューム分割]
    D -->|分岐点以上| F[既定値で足りる]
...
移行先のボリュームの inode 容量と使用数は、次の読み取り専用コマンドで確認できます（ONTAP REST API）。CloudWatch の `FilesCapacity` / `FilesUsed` でも同じ値を読めます。
...
各ボリュームの `files.maximum`（inode の総数）と `files.used`（使用数）が返ります。`files.used / files.maximum` の比が、容量の使用率より先に 100% に近づいていないかを確認します。近づいているなら、容量ではなく inode が先に尽きます。

## Read next

[デプロイタイプは後から変えられるか？](../../02-design/notes/deployment-type-is-decided-once.md)
