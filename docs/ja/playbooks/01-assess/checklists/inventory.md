...
- [ ] **最大ファイルサイズ**。**50 GiB を超えるファイルは FSx for ONTAP S3 AP 経由では投入できません。** NFS / SMB 経由にする分岐です。**縛られるのは書き込みだけで、ファイル側で作ったあとに S3 API で読むことはできます**（[方向で非対称なサイズ上限](../../../domains/data-utilization/notes/s3-access-point-constraints.md#方向で非対称なサイズ上限)）
...
- [ ] **ブロックプロトコル（iSCSI / NVMe/TCP）を使うか**。使うなら **HA ペアは 6 組が上限**です。AWS がサポート対象とするのは 6 組以下で、7 組目追加時の遷移動作は文書化されていません。**HA ペアは削除できません**
...
- [ ] **レイテンシ分布をクライアント側またはリクエスト単位のテレメトリで測った**（実測）。CloudWatch のボリューム read/write/metadata operation-time/count ペアから得られるのは平均です
