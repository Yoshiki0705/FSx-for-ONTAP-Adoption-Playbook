...
| ゲート | 何を検証するか | ネットワーク |
|---|---|---|
| `make cross-repo` | 本文に現れる sibling repo へのリンクが下の表に載っているか。表の行の引用元ファイルが実在し、実際にそのリンクを含むか。**公開している [probe 登録簿](../../agent/cross-repo-probe-contract.txt) がこの表と一致するか** | 不要 |
...
- **役割** — 発火が何を意味するか。`retraction` か `reread` のいずれかで、**他の値はゲートが拒否します。** 判定の基準は[発火の意味が 1 通りでない probe の扱い](#発火の意味が-1-通りでない-probe-の扱い)にあり、値は [probe 登録簿](../../agent/cross-repo-probe-contract.txt)として引用先へ公開されます
...
| `docs/en/domains/block-storage/notes/paths-are-the-failover-mechanism.md` | `S3-Burst-on-ONTAP-Files` | `docs/ja/verification/perf-matrix-results.md` | `500〜592 MB/s に収まる` | `reread` | 同じ主張の EN 版からの引用。ファイルプロトコルの単一接続が 625 MBps に届かないこと |
| `docs/en/domains/block-storage/notes/paths-are-the-failover-mechanism.md` | `S3-Burst-on-ONTAP-Files` | `docs/ja/verification/perf-matrix-results.md` | `iSCSI の 1 本はその約 2 倍を運んだ` | `retraction` | 同じ主張の EN 版からの引用。ブロックの単一接続がファイル側の約 2 倍だったこと |
| `docs/en/domains/block-storage/notes/paths-are-the-failover-mechanism.md` | `S3-Burst-on-ONTAP-Files` | `docs/ja/verification-status.md` | `ANA は 2 つのコントローラを 1 デバイスに統合し、逐次読みを 23% 上げた` | `retraction` | 同じ主張の EN 版からの引用。ANA が有効なカーネルで 2 コントローラが 1 デバイスに統合されること |
| `docs/en/domains/block-storage/notes/paths-are-the-failover-mechanism.md` | `S3-Burst-on-ONTAP-Files` | `docs/ja/verification-status.md` | `6.1.186-228.374 / 6.12.103-127.188 / 6.18.48-107.148` | `retraction` | 同じ主張の EN 版からの引用。AL2023 のカーネル 3 系統で `CONFIG_NVME_MULTIPATH` が未設定だったこと |
...
| `docs/en/domains/block-storage/notes/when-shared-block-changes-the-design.md` | `VMware-Migration-EC2-ONTAP` | `docs/ja/tco-comparison.md` | `逆転点はスループット容量に比例します。` | `retraction` | 同じ主張の EN 版からの引用。Amazon EBS と FSx for ONTAP の単価の逆転点が必要スループット容量に依存し、定数化できないこと |
...
<a id="引用先が自分で検査するための-probe-登録簿"></a>

### 引用先による検査のための probe 登録簿
...
**[probe 登録簿](#引用先による検査のための-probe-登録簿)はこの規則の例外ではありません。** 転記では
...
| 向き | 追跡 | ゲート |
|---|---|---|
| outbound（こちらが他所を pin） | この索引と [probe 登録簿](../../agent/cross-repo-probe-contract.txt) | `make cross-repo` / `make cross-repo-external` |
| **inbound（他所がこちらを pin）** | [inbound probe 登録簿](../../agent/inbound-probe-contract.txt) | **`make inbound-probes`** |
| 外部から引用されている見出し | [外部参照アンカーのスナップショット](../../agent/external-anchor-contract.txt) | `make anchors` |
...
**網羅は下限です。** 発見は各 sibling が**公開している probe 登録簿**を読み、sibling の一覧は本文中の
リポジトリ参照から導出します（固定の一覧を持ちません）。**probe 登録簿の不在は probe の不在では
...
| 5 | `python3 tools/check_cross_repo.py --write-contract` を実行する | **引用先が読むのは公開した probe 登録簿で、この表ではありません** |
| 6 | 条件を本文に併記する。引用先だけに置かない | 読者が引用先を開かずに誤用します |
| 7 | `make cross-repo` を実行する | リンクと表の対応、および probe 登録簿が表と一致すること |
