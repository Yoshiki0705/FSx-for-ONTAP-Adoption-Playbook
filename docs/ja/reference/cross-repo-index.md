...
| `retraction` | それ以外 | **こちら側の記述が根拠を失った。** ただし**語は判定を指していて、原因を指していません** — 撤回・移動・文言の変更・（不在を運ぶ probe なら）不在が偽になったこと、の 4 つが同じ消え方をします |
...
**そのぶん失敗出力に原因を列挙します。** 不在が偽になった場合、起きたのは**引用先が何かを
知ったこと**で撤回ではありません。**`retraction` という語だけを見ると逆に読めるので、
4 つの原因をメッセージに書いてあります。** 役割値を 3 つ目に増やすと、公開した契約を読む側で
未知の値になるため、**語彙を増やさずメッセージで解いています。**
