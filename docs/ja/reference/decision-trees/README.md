...
| [ウイルス対策の適用範囲をどこまでにするか](vscan-antivirus-scope.md) | AD 参加の有無・対象プロトコル・共有の設定・スキャン不能時の扱いで構成が先に決まる順序をたどる。**終端にベンダー名は現れません** / Following the order in which AD membership, protocol, share settings, and the behaviour when scanning is unavailable settle the configuration. **No terminal names a vendor** |
