...
graph TD
    ...
    GRAN1 -->|足りない| DASH{必要な指標の個別実装を<br/>保守し続けられるか}

    DASH -->|保守できない: 既製画面が必要| CHK{必要な画面が<br/>非サポート 10 種に<br/>入っていないか}
    DASH -->|保守できる| R4["経路 4: ONTAP REST 直叩き"]
...
| 4 | 必要な指標の個別実装を保守し続けられるか | **経路 4**（ONTAP REST 直叩き） | 条件 5 へ |
...
**条件 4 で個別実装を保守し続けられるなら経路 4 に振れる理由**は、収集基盤を持つコストが得られる値に見合わなくなる場合があるためです。逆に対象が増えるほど、自作の保守コストが既製の運用コストを上回ります。**分岐の閾値は台数ではなく「保守を続けられるか」です。**
...
| 4 | 必要な指標の個別実装を保守し続けられるか確認する | 条件 4 の答え |
