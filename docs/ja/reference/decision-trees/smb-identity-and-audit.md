...
graph TD
    ...
    Q1 -->|利用できる| Q2{"アカウント数と<br/>異動に伴う増減頻度"}
    Q2 -->|少数かつ固定的| WG2["ワークグループでも成立<br/>台帳での管理が前提"]
    Q2 -->|数十以上かつ増減が頻繁| AD[AD 参加]
    Q2 -->|一方だけ該当| COMPARE["監査運用と AD 運用の<br/>コストを比較"]
...
**選び方**: アカウント数と増減の頻度で決まります。**数十以上あり異動に伴う増減が頻繁なら AD 参加**、**少数で固定的ならワークグループと台帳管理**が目安です。どちらか一方だけに該当する場合は、監査を有効化して棚卸しする運用コストと、AD を運用するコストを比較します。ワークグループを選ぶ場合も同じ比較が必要です。**どちらの枝にも運用コストがあり、置き場所が違うだけです。**
