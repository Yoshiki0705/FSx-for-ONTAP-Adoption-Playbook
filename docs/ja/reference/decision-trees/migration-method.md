...
graph TD
    ...
    P -->|NFS| N{切替直前まで<br/>差分同期を続ける<br/>必要があるか}
    P -->|SMB| M{ACL 保持が必要か}
    P -->|両方| B[プロトコルごとに<br/>方式を分けて検討]

    N -->|必要| N1[AWS DataSync<br/>差分同期を繰り返す]
    N -->|不要: 停止中に全量コピー可能| N2[ホスト側コピー<br/>rsync / tar]
...
| 3 | 切替直前まで差分同期を続ける必要があるか | 必要なら差分同期を繰り返せる方式を選ぶ。停止中に全量コピーできるならホスト側コピーも候補になる |
