...
| NAS 移行とデータの可視化 | 2 | **あり** | [NAS 移行とデータの可視化](#nas-移行とデータの可視化) |
| ブロックを含むサーバー移行 | 1 | なし | [ブロックを含むサーバー移行](#ブロックを含むサーバー移行) |
| 既存バックアップ基盤への統合 | 1 | **あり** | [既存バックアップ基盤への統合](#既存バックアップ基盤への統合) |
...
**そして「安くする」目的では、外部ツールのアーカイブと FSx for ONTAP の容量プール階層が同じ列に見えますが、順序を持ちます。** 公開されている削減率の基準が自社の旧環境であることも、引く前に確認が要ります。

- 判断ノート: [外部ツールのアーカイブと容量プール階層は代替ではなく順序を持つ](../domains/cost/notes/archiving-and-tiering-are-ordered-not-alternatives.md)
...
**Datadobi StorageMAP はベンダー告知のみで、判断ノートを立てていません。** 掲載基準は満たしますが、制約を整理するに足る一次情報に到達していません。
...
**そして「対応」の書き方が経路ごとに違います。** 一方は非対応の明示、もう一方は対応システムの列挙に載っていないという状態で、**後者を非対応と読むことはできません。**

**AWS ネイティブの手立てが並びます。** FSx for ONTAP のボリュームバックアップと AWS Backup、および ONTAP の Snapshot と SnapMirror。

- 判断ノート: [第三者バックアップ製品が届く経路は AWS の API 側ではない](../domains/data-protection/notes/third-party-backup-reaches-it-by-another-route.md)
