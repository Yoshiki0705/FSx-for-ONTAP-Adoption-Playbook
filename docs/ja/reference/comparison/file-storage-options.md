...
**この節は選定の根拠に使えません。** sibling 側で測定済みなのは、共有の可視性・単一チャネル・SMB Multichannel・7 シナリオ比較までです。

| 未測定 | 何が言えないか |
|---|---|
| 台数試験（8 台） | 台数を増やしたときの合計スループットの伸び |
| 15 分の持続書き込み | バーストではない持続性能 |
| キャッシュ制御下（重ならない領域の読み取り） | ディスク側の上限に当たったときの挙動 |

**測定済みの範囲についても 2 点の留保があります。**

- RSS 対応 NIC 1 枚で観測したチャネル数は 4 で、`max_connections_per_session` を 32 にしても 4 で止まりました。**なぜそうなったのかは sibling 側でも確認されていません。4 が製品の上限だと読まないでください**
- NFS 16 接続の列と SMB Multichannel の列は `キャッシュの温度が揃っていない` と記録されています。**この 2 列から SMB と NFS の優劣を取れません**

3 項目が揃った時点で sibling から連絡が来る取り決めになっています。**それまでこの節の内容を選定やエージェントの適合判断の根拠にしないでください。** 測っていないことを根拠にするのが、いちばん害が大きい形です。
