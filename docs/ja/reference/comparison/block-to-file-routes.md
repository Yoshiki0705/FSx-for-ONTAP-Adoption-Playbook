...
> **この表の「実際」列には、他リポジトリのゲートが参照している文が含まれます。** 該当する文字列は
> [inbound-probe-contract.txt](../../../agent/inbound-probe-contract.txt) にあり、**この表を書き直す前に
> そちらを確認してください。** 文字列が消えると相手側では**主張の撤回として報告されます** — こちらの
> 編集だとは伝わりません。そして**同じ文字列をこの文書のどこかにもう一度書くと、相手のゲートは緑の
> まま何も守らなくなります。** ここで引用せず所在だけを示しているのはそのためです。`make inbound-probes`
> が両方を検出します。
