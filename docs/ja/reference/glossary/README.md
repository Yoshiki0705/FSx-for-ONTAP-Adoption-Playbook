...
| `volume rehost` | ボリュームを SVM 間で再割り当てする操作。SnapMirror のコピーを伴わない。**変わるのは所有 SVM だけで中身は変わらず、LUN は unmapped で保持される。セキュリティスタイルも保持される**（実測）。FlexClone とは排他で、split が前提。**disruptive で、7 種の設定が失われる — ただし Snapshot ポリシーは失われるのではなく `default` に戻る**（実測）。**REST では `POST /api/private/cli/volume/rehost`。ボリュームの `svm` の書き換えは HTTP 400** / Reassigns a volume between SVMs without a SnapMirror copy. **Only the owning SVM changes — contents do not, LUNs are preserved but unmapped, and the security style is preserved** (measured). Mutually exclusive with FlexClone, so a split is a prerequisite. **Disruptive, and seven categories of configuration are lost — but the snapshot policy is reset to `default` rather than lost** (measured). **Over REST it is `POST /api/private/cli/volume/rehost`; rewriting the volume's `svm` returns HTTP 400** ([`volume rehost` が変えるものと変えないもの](../../domains/block-storage/notes/volume-rehost-changes-ownership-not-contents.md)) |
...
| Volume recovery queue | 削除したボリュームが一定時間留まる場所。**既定で 12 時間以上**保持され、`<名前>_<データセット ID>` に改名されて `volume show` から隠れる。**その間 FlexClone の関係は残り、親ボリュームを削除できない。** `volume recovery-queue purge` で即座に解消する / Where a deleted volume waits. **Retained for at least 12 hours by default**, renamed to `<name>_<data set ID>` and hidden from `volume show`. **A FlexClone relationship persists meanwhile and the parent volume cannot be deleted.** `volume recovery-queue purge` clears it immediately ([NetApp](https://docs.netapp.com/us-en/ontap/volumes/protection-accidental-volume-deletion-concept.html)) |
| Portset | igroup に対して、LUN を見せる LIF を限定する設定。Selective LUN Map の上に載る追加の絞り込み。**除外したパスはホストから消えず `faulty` として残るので、SCSI デバイスの削除まで必要** / A setting restricting which LIFs expose a LUN to an igroup, layered on top of Selective LUN Map. **The excluded path does not disappear from the host but remains `faulty`, so the SCSI device must be deleted** ([igroup の外側にある 2 つの制御](../../domains/block-storage/notes/igroups-are-not-the-only-access-control.md)) |
| CHAP | iSCSI のイニシエータを認証する仕組み。**既定は `none` なので、igroup だけでは IQN を名乗れば届く。** 失敗は `error (24 ... authorization failure)` として出る / The mechanism authenticating an iSCSI initiator. **The default is `none`, so an igroup alone admits anyone who can name a listed IQN.** A failure surfaces as `error (24 ... authorization failure)` ([NetApp](https://docs.netapp.com/us-en/ontap/san-admin/iscsi-authentication-concept.html)) |
| Consistency group | 複数ボリュームを 1 単位として扱う仕組み。**`-write-fence true` で全ボリュームを同一時点に確定させる。** 複数 LUN にまたがる DB を止めずに取るための道具 / A mechanism treating several volumes as one unit. **`-write-fence true` fixes every member at one instant.** The tool for capturing a database spanning several LUNs without stopping it ([LUN に載せた DB は静止させずに復旧した](../../domains/block-storage/notes/a-database-on-luns-recovers-without-quiescing.md)) |
| `CONFIG_NVME_MULTIPATH` | NVMe のネイティブ multipath を有効にするカーネル構成。**Amazon Linux 2023 では無効で、1 つの namespace が同じ `wwid` の 2 デバイスとして現れ、フェイルオーバーが効かない** / The kernel option enabling native NVMe multipath. **Disabled on Amazon Linux 2023, so one namespace appears as two devices with the same `wwid` and failover does not work** ([パスはフェイルオーバーの仕組みそのもの](../../domains/block-storage/notes/paths-are-the-failover-mechanism.md#実測したフェイルオーバー)) |
...
| LDAP | ユーザー・グループ・netgroup を格納するディレクトリへのアクセスプロトコル。データ保護ページは LDAP ドメインに参加した SVM での Kerberos 転送時暗号化を挙げるが、詳細ページは NFS Kerberos を Microsoft Active Directory 参加として説明している / A protocol for accessing a directory holding users, groups, and netgroups. The data-protection page names Kerberos encryption for an SVM joined to a domain over LDAP, while the detailed NFS section describes Microsoft Active Directory ([AWS](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/data-protection.html)) |
| Kerberos | NFSv3・NFSv4 の転送時暗号化に使える認証方式。詳細ページは Microsoft Active Directory に参加した SVM の子ボリュームを対象に説明する / An authentication mechanism available for NFSv3 and NFSv4 encryption in transit. The detailed page describes child volumes of an SVM joined to Microsoft Active Directory ([AWS](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/encryption-in-transit.html)) |
