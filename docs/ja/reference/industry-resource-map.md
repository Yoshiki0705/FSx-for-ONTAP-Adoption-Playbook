...
**末尾の 4 行は業種でもワークロードの形でもありません。** サイバーレジリエンス・可観測性・データレイクは関心事で、ファイルシステム運用は読者の役割です。**ファイルシステムに対して行う操作（容量の均し、階層化ポリシー、Snapshot の整理、スループットの調整）は、走らせているものではなく、走っているものに対してすることなので、形で引けません。** 運用の行はそのために置いてあります。
...
| ファイルシステム運用（業種横断） | [運用](../playbooks/05-operate/) | [コスト](../domains/cost/) | **下げる操作が請求に効くかは、確保と消費のどちらで課金されているかで決まります**（[請求が想定より高いとき](decision-trees/cost-higher-than-expected.md)）。容量・階層化・効率化の推奨値は運用の目安であって、ベンチマークでも請求額でもありません |
