...
| 項目 | 値 | 出典 | 検証日 | 備考 |
|---|---|---|---|---|
| ONTAP バージョン | `9.17.1P7D1` | 実測 | 2026-08-07 | ビルド日時も併せて返ります |
| ONTAP REST の認証 | `fsxadmin` の Basic 認証 | 実測 | 2026-08-07 | 未認証は `401` |
...
| 項目 | 値 | 出典 | 検証日 | 備考 |
|---|---|---|---|---|
| 1 ボリュームあたりの Snapshot 上限 | **1,023 個** | 実測 | 2026-08-07 | 1,024 個目が `500` で拒否。`num_records` は 1,023 |
| 上限到達時のエラー | `Cannot exceed maximum number of snapshots.` | 実測 | 2026-08-07 | ONTAP エラーコード `525062` |
| 1,023 個分の Snapshot メタデータ | **156,659,712 B**（約 149 MiB） | 実測 | 2026-08-07 | **空のボリューム**で。1 個あたり約 150 KiB |
| Snapshot 予約の既定 | **5%** | 実測 | 2026-08-07 | 8 GiB では 429,494,272 B |
| 予約に対する使用率 | 36% | 実測 | 2026-08-07 | 1,023 個時点 |
...
| Snapshot 上限 / SnapLock 監査ログの解除 | 上記と同じ一方 | **`9.17.1P7D1`** | ONTAP REST API | 2026-08-07 |
