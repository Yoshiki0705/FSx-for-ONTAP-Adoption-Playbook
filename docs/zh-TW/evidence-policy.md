...
| `documented` | 廠商或 AWS 官方文件中有記載。**或者，擁有測量環境的其他專案的紀錄中有記載**（見下文） | `source`（URL 或文件名稱） | 可作為第一手資訊對待。但需注意版本與區域差異 |
...
**轉載其他專案測量的數值同樣是 `documented`。** `verified` 主張的是「本儲存庫的作者在該環境中重現過」，引用並不滿足這一點。**但引用附帶 3 個特有條件。**

| 條件 | 理由 |
|---|---|
| **完整轉載全部測量條件** | 缺少條件的數值無法用於設計。世代、容量、IOPS、快取狀態、用戶端執例類型、並行度都要寫 |
| **連引用來源自稱未測量的範圍一併轉載** | **引用可以只取方便的部分。** 丟掉來源列出的空白，就等於偽裝確定性 |
| **登錄到[跨專案引用索引](../ja/reference/cross-repo-index.md)（日本語）** | **引用會靜默腐爛。** 來源撤回主張時，這邊的敘述不會改變。登錄後 `make cross-repo-external` 會檢出撤回 |
...
- `evidence: verified` 時存在 `region`（**若無法寫明環境，則說明等級選錯了**）
- frontmatter 的鍵包含在已知集合中（**拼寫錯誤等同於沒有該值，卻仍會被讀者看到**）
