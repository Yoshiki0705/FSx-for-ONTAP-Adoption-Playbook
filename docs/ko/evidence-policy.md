...
| `documented` | 벤더 또는 AWS 공식 문서에 기재가 있다. **또는 측정 환경을 보유한 다른 프로젝트의 기록에 기재가 있다**(아래 참조) | `source`(URL 또는 문서명) | 1차 정보로 취급할 수 있다. 다만 버전·리전 차이에 주의 |
...
**다른 프로젝트가 측정한 수치를 전재한 경우도 `documented`입니다.** `verified`가 주장하는 것은 「이 리포지토리의 저자가 그 환경에서 재현했다」는 것이므로, 인용은 이를 충족하지 않습니다. **다만 인용에는 고유한 조건이 3 가지 붙습니다.**

| 조건 | 이유 |
|---|---|
| **측정 조건을 전 항목 전재한다** | 조건이 빠진 수치는 설계에 쓸 수 없습니다. 세대·용량·IOPS·캐시 상태·클라이언트 인스턴스 타입·병렬도까지 |
| **인용원이 미측정이라고 밝힌 범위도 전재한다** | **인용은 편리한 부분만 꺼낼 수 있습니다.** 인용원이 든 공백을 빠뜨리면 확실성을 위장하게 됩니다 |
| **[프로젝트 간 인용 색인](../ja/reference/cross-repo-index.md)(日本語)에 등록한다** | **인용은 조용히 썩습니다.** 인용원이 주장을 철회해도 이쪽 기술은 변하지 않습니다. 등록하면 `make cross-repo-external`이 철회를 검출합니다 |
