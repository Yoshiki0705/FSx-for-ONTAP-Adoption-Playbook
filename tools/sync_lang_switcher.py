...
Two checks live here because they are two halves of the same problem: keeping a multi-language tree
navigable without hand-maintaining it.

1. The switcher block. Eight languages times a growing file count is more switcher links than anyone
   will keep correct by hand, and a hard-coded eight-language switcher in a file that has two
   translations is seven broken links. So the block is generated from what exists on disk.

2. Links that prefer another language. `check_links.py` proves a target resolves; it cannot see that
   docs/en/... sent an English reader into docs/ja/... for a page that exists in English. That is
   the failure translations introduce over time: a page gets translated and the links that were
   legitimately falling back to Japanese are never repointed. The fallback still resolves, so no
   existing check notices.
...
def split_rel(rel: str) -> tuple[str, str] | None:
    """Split a repo-relative path into (lang, subpath), or None when it is not localized content."""
    if rel == HUB:
        return "ja", HUB
...
def eligible() -> list[str]:
    ...
    Underscore-prefixed components are scaffolding (`_template/`, `_assets/`) and are skipped, the
    same convention the other validators use.
    """
...
def iter_links(text: str):
    """Yield (lineno, line, target) for inline links outside fences and outside switcher blocks."""
    lines = text.splitlines()
    inside = set()
