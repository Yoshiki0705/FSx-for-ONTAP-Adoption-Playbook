...
| # | 모듈 | 다루는 질문 |
|---|---|---|
| 01 | [`01-assess/`](playbooks/01-assess/) | 현행 NAS에 무엇이 있고, 무엇이 마이그레이션의 제약이 되는가 |
| 02 | [`02-design/`](playbooks/02-design/) | 어떤 구성·용량·스루풋·보호 방식을 선택하는가 |
| 03 | [`03-migrate/`](playbooks/03-migrate/) | 어떤 방식으로, 어떻게 전환하고, 어떻게 되돌리는가 |
| 04 | [`04-build/`](playbooks/04-build/) | IaC·자동화·재현 가능한 구축을 어떻게 구성하는가 |
| 05 | [`05-operate/`](playbooks/05-operate/) | 모니터링·용량·장애 대응·변경 관리를 어떻게 운영하는가 |
| 06 | [`06-optimize/`](playbooks/06-optimize/) | 성능과 비용을 어디까지 다듬을 것인가 |
...
| 모듈 | 다루는 질문 |
|---|---|
| [`data-protection/`](domains/data-protection/) | Snapshot / SnapMirror / SnapLock / 백업·랜섬웨어 대응 |
| [`data-utilization/`](domains/data-utilization/) | 분석·AI/RAG·S3 API 경유 데이터 활용 |
| [`security-governance/`](domains/security-governance/) | 암호화·감사·권한 설계·규제 대응의 관점 |
| [`performance/`](domains/performance/) | 스루풋 설계·레이턴시·캐시·공유 대역 |
| [`cost/`](domains/cost/) | 용량·티어링과 견적 대비 실측의 차이 |
| [`multiprotocol-identity/`](domains/multiprotocol-identity/) | NFS / SMB 공존·Active Directory 연계·ID 매핑 |

### 횡단 참조 — `reference/`

| 디렉터리 | 개요 |
|---|---|
| [`decision-trees/`](reference/decision-trees/) | 선택 플로차트(마이그레이션 방식·보호 방식·프로토콜) |
| [`comparison/`](reference/comparison/) | 선택지 비교 매트릭스(트레이드오프를 대칭으로 기재) |
| [`limits/`](reference/limits/) | 상한값·쿼터와 그 출처·검증일 |
| [`glossary/`](reference/glossary/) | ONTAP / AWS 용어의 정의 |
...
`evidence`의 4단계는 독자가 "어디까지 신뢰하고 사용할 수 있는지"를 판단하기 위한 구분입니다. 자세한 내용은 [지식 분류 정책](docs/en/evidence-policy.md)을 참조하세요.
...
사례는 "무엇이 문제였고, 어떻게 판단했으며, 결과가 어땠는지"를 **일반화된 교훈**으로 씁니다. 템플릿은 [`case-studies/_template/`](case-studies/_template/)에 있습니다. 공개 전 확인은 `make audit`으로 자동화되어 있습니다.
...
| [`reference/limits/`](reference/limits/) | 상한값을 출처·검증일과 함께 구조화 |
...
Issue / Pull Request를 환영합니다. 집필 규약은 [CONTRIBUTING.md](CONTRIBUTING.md), 지식 분류 기준은 [지식 분류 정책](docs/en/evidence-policy.md)을 참조하세요.
