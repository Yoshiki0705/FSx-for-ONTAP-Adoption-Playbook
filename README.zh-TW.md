...
| # | 模組 | 所解答的問題 |
|---|---|---|
| 01 | [`01-assess/`](playbooks/01-assess/) | 現有 NAS 上有什麼，什麼會成為遷移的限制 |
| 02 | [`02-design/`](playbooks/02-design/) | 選擇哪種組態、容量、傳輸量與保護方式 |
| 03 | [`03-migrate/`](playbooks/03-migrate/) | 採用哪種方式、如何切換、如何回復 |
| 04 | [`04-build/`](playbooks/04-build/) | 如何組織 IaC、自動化與可重現的建置 |
| 05 | [`05-operate/`](playbooks/05-operate/) | 如何運作監控、容量、故障應變與變更管理 |
| 06 | [`06-optimize/`](playbooks/06-optimize/) | 效能與成本要調校到什麼程度 |
...
| 模組 | 所解答的問題 |
|---|---|
| [`data-protection/`](domains/data-protection/) | Snapshot / SnapMirror / SnapLock / 備份與勒索軟體應變 |
| [`data-utilization/`](domains/data-utilization/) | 分析、AI/RAG、經由 S3 API 的資料活用 |
| [`security-governance/`](domains/security-governance/) | 加密、稽核、權限設計、法規議題的思考方式 |
| [`performance/`](domains/performance/) | 傳輸量設計、延遲、快取、共用頻寬 |
| [`cost/`](domains/cost/) | 容量、分層，以及估算與實測之間的落差 |
| [`multiprotocol-identity/`](domains/multiprotocol-identity/) | NFS / SMB 共存、Active Directory 整合、ID 對應 |

### 橫向參考 — `reference/`

| 目錄 | 內容 |
|---|---|
| [`decision-trees/`](reference/decision-trees/) | 選擇流程圖（遷移方式、保護方式、協定） |
| [`comparison/`](reference/comparison/) | 選項比較矩陣（對稱地列出取捨） |
| [`limits/`](reference/limits/) | 上限與配額，附出處與驗證日期 |
| [`glossary/`](reference/glossary/) | ONTAP / AWS 術語定義 |
...
`evidence` 的四個級別用於讓讀者判斷「該敘述可以信賴到什麼程度」。詳情參見[知識分級政策](docs/en/evidence-policy.md)。
...
案例以**一般化的教訓**來撰寫：問題是什麼、如何判斷、結果如何。範本位於 [`case-studies/_template/`](case-studies/_template/)。公開前檢查已透過 `make audit` 自動化。
...
| [`reference/limits/`](reference/limits/) | 將上限值與出處、驗證日期一併結構化 |
...
歡迎提交 Issue / Pull Request。撰寫規約見 [CONTRIBUTING.md](CONTRIBUTING.md)，知識分級標準見[知識分級政策](docs/en/evidence-policy.md)。
