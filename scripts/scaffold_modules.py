...
RELATED_JA = {
    "playbooks": "- [テーマ軸で探す](../../docs/ja/navigation.md#テーマ軸--domains)\n"
    "- [移行方式 決定ツリー](../../reference/decision-trees/migration-method.md)",
    "domains": "- [ライフサイクル軸で探す](../../docs/ja/navigation.md#ライフサイクル軸--playbooks)\n"
    "- [比較マトリクス](../../reference/comparison/)",
}
RELATED_EN = {
    "playbooks": "- [Browse by topic](../../docs/en/navigation.md#topic-axis--domains)\n"
    "- [Migration Method Decision Tree](../../reference/decision-trees/migration-method.md)",
    "domains": "- [Browse by lifecycle](../../docs/en/navigation.md#lifecycle-axis--playbooks)\n"
...
def main() -> int:
    ...
    for group, directory, ja_title, en_title, ja_lead, en_lead, questions in MODULES:
        module = ROOT / group / directory
        (module / "notes").mkdir(parents=True, exist_ok=True)
        (module / "checklists").mkdir(parents=True, exist_ok=True)

        for filename, template, title, lead, related, index in (
            ("README.md", JA, ja_title, ja_lead, RELATED_JA[group], 0),
            ("README.en.md", EN, en_title, en_lead, RELATED_EN[group], 1),
        ):
