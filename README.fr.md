...
|---|---|---|
| Comprendre comment parcourir ce dépôt | [Guide de navigation](docs/en/navigation.md) | 3 min |
| Décider s'il faut migrer, et comment | [Arbre de décision : méthode de migration](reference/decision-trees/migration-method.md) | 10 min |
| Inventorier le NAS existant | [Playbook 01 — Évaluation](playbooks/01-assess/) | 30 min |
| Concevoir la configuration cible | [Playbook 02 — Conception](playbooks/02-design/) | 60 min |
| Exécuter la migration | [Playbook 03 — Migration](playbooks/03-migrate/) | — |
| Mettre en place l'exploitation | [Playbook 05 — Exploitation](playbooks/05-operate/) | — |
| Concevoir la protection des données (Snapshot / SnapMirror / WORM) | [Domaine — Protection des données](domains/data-protection/) | 30 min |
| Comparer aux autres options de stockage | [Matrices de comparaison](reference/comparison/) | 15 min |
| Lire des cas d'usage (anonymisés) | [Case Studies](case-studies/) | 10 min |
...
|---|---|---|
| 01 | [`01-assess/`](playbooks/01-assess/) | Ce qui existe sur le NAS actuel, et ce qui contraindra la migration |
| 02 | [`02-design/`](playbooks/02-design/) | Quelle configuration, capacité, débit et méthode de protection retenir |
| 03 | [`03-migrate/`](playbooks/03-migrate/) | Quelle méthode employer, comment basculer, comment revenir en arrière |
| 04 | [`04-build/`](playbooks/04-build/) | Comment structurer l'IaC, l'automatisation et des builds reproductibles |
| 05 | [`05-operate/`](playbooks/05-operate/) | Comment mener supervision, capacité, réponse aux incidents et gestion du changement |
| 06 | [`06-optimize/`](playbooks/06-optimize/) | Jusqu'où pousser l'optimisation de la performance et du coût |
...
| Module | Question traitée |
|---|---|
| [`data-protection/`](domains/data-protection/) | Snapshot / SnapMirror / SnapLock / sauvegarde et préparation aux rançongiciels |
| [`data-utilization/`](domains/data-utilization/) | Analytique, IA/RAG, accès via l'API S3 |
| [`security-governance/`](domains/security-governance/) | Chiffrement, audit, conception des droits, approche des charges réglementées |
| [`performance/`](domains/performance/) | Dimensionnement du débit, latence, cache, bande passante partagée |
| [`cost/`](domains/cost/) | Capacité, tiering, et l'écart entre estimations et mesures |
| [`multiprotocol-identity/`](domains/multiprotocol-identity/) | Coexistence NFS / SMB, intégration Active Directory, mappage d'identités |

### Référence transversale — `reference/`

| Répertoire | Contenu |
|---|---|
| [`decision-trees/`](reference/decision-trees/) | Organigrammes de choix (méthode de migration, protection, protocole) |
| [`comparison/`](reference/comparison/) | Matrices de comparaison (compromis énoncés de façon symétrique) |
| [`limits/`](reference/limits/) | Limites et quotas, avec source et date de vérification |
| [`glossary/`](reference/glossary/) | Terminologie ONTAP et AWS |
...
```text
<module>/
├── README.md          # Hub du module (JA)
├── README.en.md       # Hub du module (EN)
...
Les quatre niveaux d'`evidence` permettent au lecteur de juger jusqu'où une note peut être utilisée. Voir la [politique de niveaux de preuve](docs/en/evidence-policy.md).
...
Les cas d'usage sont rédigés comme des **enseignements généralisés** : quel était le problème, comment la décision a été prise, quel a été le résultat. Le modèle se trouve dans [`case-studies/_template/`](case-studies/_template/). Les contrôles avant publication sont automatisés par `make audit`.
...
| [`reference/limits/`](reference/limits/) | Limites structurées avec source et date de vérification |
...
Les Issues et Pull Requests sont bienvenues. Voir [CONTRIBUTING.md](CONTRIBUTING.md) pour les conventions de rédaction et la [politique de niveaux de preuve](docs/en/evidence-policy.md) pour les critères de classement.
