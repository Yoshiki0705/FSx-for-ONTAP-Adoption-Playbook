...
| # | 模块 | 所解答的问题 |
|---|---|---|
| 01 | [`01-assess/`](playbooks/01-assess/) | 现有 NAS 上有什么，什么会成为迁移的约束 |
| 02 | [`02-design/`](playbooks/02-design/) | 选择哪种配置、容量、吞吐量与保护方式 |
| 03 | [`03-migrate/`](playbooks/03-migrate/) | 采用哪种方式、如何切换、如何回滚 |
| 04 | [`04-build/`](playbooks/04-build/) | 如何组织 IaC、自动化与可复现的构建 |
| 05 | [`05-operate/`](playbooks/05-operate/) | 如何运转监控、容量、故障响应与变更管理 |
| 06 | [`06-optimize/`](playbooks/06-optimize/) | 性能与成本要调优到什么程度 |
...
| 模块 | 所解答的问题 |
|---|---|
| [`data-protection/`](domains/data-protection/) | Snapshot / SnapMirror / SnapLock / 备份与勒索软件应对 |
| [`data-utilization/`](domains/data-utilization/) | 分析、AI/RAG、经由 S3 API 的数据利用 |
| [`security-governance/`](domains/security-governance/) | 加密、审计、权限设计、合规议题的思考方式 |
| [`performance/`](domains/performance/) | 吞吐量设计、延迟、缓存、共享带宽 |
| [`cost/`](domains/cost/) | 容量、分层，以及估算与实测之间的差异 |
| [`multiprotocol-identity/`](domains/multiprotocol-identity/) | NFS / SMB 共存、Active Directory 集成、ID 映射 |

### 横向参考 — `reference/`

| 目录 | 内容 |
|---|---|
| [`decision-trees/`](reference/decision-trees/) | 选择流程图（迁移方式、保护方式、协议） |
| [`comparison/`](reference/comparison/) | 选项比较矩阵（对称地列出权衡） |
| [`limits/`](reference/limits/) | 上限与配额，附出处与验证日期 |
| [`glossary/`](reference/glossary/) | ONTAP / AWS 术语定义 |
...
`evidence` 的四个级别用于让读者判断"该记述可以信赖到什么程度"。详情参见[知识分级政策](docs/en/evidence-policy.md)。
...
案例以**一般化的教训**来撰写：问题是什么、如何判断、结果如何。模板位于 [`case-studies/_template/`](case-studies/_template/)。公开前检查已通过 `make audit` 自动化。
...
| [`reference/limits/`](reference/limits/) | 将上限值与出处、验证日期一并结构化 |
...
欢迎提交 Issue / Pull Request。撰写规约见 [CONTRIBUTING.md](CONTRIBUTING.md)，知识分级标准见[知识分级政策](docs/en/evidence-policy.md)。
